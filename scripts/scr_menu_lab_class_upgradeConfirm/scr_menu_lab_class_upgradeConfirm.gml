...
if(global.labObj.level >= 30 && !DEBUG){
    ...
    (_m.menu_x > 0 && global.labObj.grd_skills[# _m.menu_x + -1,_m.menu_y] >= 5) ||
    (global.labObj.prototype && _m.menu_x > 2)
){
    scr_menu_confirmWindow("menu/lab/class1/upgrade/confirm/..","Skill is already at max level.",scr_menu_backNoRefresh,noone);
}else if(global.labObj.level < (_m.menu_x + -2) * 10 && !DEBUG){
    ...
    scr_cEvent_id(global.labObj,EVENT_CLASS_FETCHTOOLTIP,_m.menu_x + -1,_m.menu_y);
