with obj_handler_dungeon{
    ...
    #region //draw map grid
    	...
    	var
    	_drawDist = !global.set_mapZoom ? 5 : 3,
    ...
    #region //draw minimap
    	...
    	if(global.set_mmSize){
        	var
        	_mm_size = 4,
        	_mm_len = global.set_mmSize,
        	_mm_x = 0,
        	_mm_y = 0,
        	_mm_xStart = _mm_x,
        	_mm_yStart = _mm_y,
        	_mm_a = .8,
        	_mm_ix = clamp(round(global.dMap_xPos) + -(_mm_len / 2),0,max(ds_grid_width(global.grd_dMap_terrain) + -_mm_len,0)),
        	_mm_iy = clamp(round(global.dMap_yPos) + -(_mm_len / 2),0,max(ds_grid_height(global.grd_dMap_terrain) + -_mm_len,0));
